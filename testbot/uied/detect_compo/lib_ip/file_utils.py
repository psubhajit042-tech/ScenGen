...
def save_corners_json(file_path, compos, img_shape=None):
    """
    Save component bounding boxes to JSON.

    Parameters
    ----------
    file_path  : destination .json path
    compos     : list of detected component objects (may be empty)
    img_shape  : (height, width[, channels]) tuple — required when compos is
                 empty so downstream consumers always find 'img_shape' in the
                 output.  When compos is non-empty the shape is read from the
                 first element as before.
    """
    os.makedirs(os.path.dirname(file_path), exist_ok=True)

    if not compos:
        # No components detected — write a valid empty result so merge.py
        # can still read 'img_shape' without a KeyError.
        shape = list(img_shape) if img_shape is not None else [0, 0, 3]
        with open(file_path, 'w') as f:
            json.dump({'img_shape': shape, 'compos': []}, f, indent=4)
        return
